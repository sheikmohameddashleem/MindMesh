...
const Chat = (props: any) => {
  ...
  useEffect(() => {
    if (el.current) {
      (el.current as HTMLElement).scrollIntoView({
        block: "start",
        behavior: "smooth",
      });
    }
    ...
    submitAnswer(answer, submitCalls)
      .then((response: any) => {
        ...
        if (response.data.hasOwnProperty("score")) {
          console.log(
            "yes ended",
            "response.data.hasOwnProperty('score')",
            response.data.hasOwnProperty("score")
          );
    ...
    <div style={{ maxWidth: "100%" }}>
      <div style={{ overscrollBehavior: "auto", height: "100vh" }}>
        <div className="fixed w-full bg-slate-600	text-center h-16 pt-2 text-white flex justify-between shadow-md top-0">
          <div className="my-3 text-green-100 font-bold text-lg tracking-wide text-center">
            @MindMesh
          </div>
        </div>
          ...
          {messages?.map((message) => (
            <div
              ...
              style={{
                display: "flex",
                justifyContent:
                  message.creator === "user" ? "flex-end" : "flex-start",
                width: "75%",
                margin: "auto",
              }}
              key={message.time}
            >
              ...
              <div
                className="w-2.5/4 mx-4 my-2 p-2 rounded-lg text-left"
                style={{
                  backgroundColor:
                    message.creator === "user" ? "#C0BEFE" : "#E6E4E3",
                }}
              >
                {message.message}
              </div>
        ...
        >
          ...
          >
            ...
            <div className="ml-3 text-sm font-normal">
              Interview completed successfully. Click to view score.
            </div>
          </div>
        </div>
      ) : (
        <div
          className="fixed w-full flex justify-between bg-slate-600"
          style={{ bottom: "0px" }}
        >
          <textarea
            id="convert_text"
          ...
          />
          <button
            className="m-2 p-2 rounded-full bg-slate-600 focus:outline-none"
            id="click_to_convert"
            onClick={handleStartListening}
            disabled={isListening}
            style={{ backgroundColor: isListening ? "red" : "grey" }}
          >
            <svg
          ...
          <button
            className="m-2"
            style={{ outline: "none" }}
            onClick={() => handleStopListening(recognizedText)}
            disabled={!isListening}
          >
            <svg
